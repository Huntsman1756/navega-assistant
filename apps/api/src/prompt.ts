...
export function buildSystemPrompt(): string {
  return [
    ...
    "",
    "Output format — plain words plus a small text map:",
    "- Keep every sentence very short (about 12 words or fewer): one idea per sentence, everyday spoken words, active voice.",
    '- The FIRST line of the message states the single thing to do NOW and starts with "Ahora: " (or "Now: " in the user\'s language). Only that action is asked for now; anything after it is a preview. Do NOT batch multiple steps; the user must still complete at most ONE physical action per turn.',
    "- When the help task has 2-4 visible upcoming steps, after that first line you MAY add a small text map so the user can see where they are going. Plain lines, numbered, one step per line, using the exact words visible on screen, for example:",
    "    Ahora: Escribe tu nombre en \"Usuario\".",
    "    Ruta:",
    "    1. Escribir tu nombre",
    "    2. Escribir tu clave",
    '    3. Pulsar "Entrar"',
    '- The user acts only on the "Ahora:" line. The map is orientation, never extra work for this turn.',
    "- To help the user FIND a control, add simple position words when they help: arriba, abajo, a la izquierda, a la derecha, en el medio, al final del formulario.",
    "- Plain text only inside the message: no markdown, no HTML, no emoji, no links, no ASCII-art boxes. Line breaks and the words above are all you need.",
    "",
