...
it("accepts normal requests and rejects oversized declared and streamed bodies before provider", async () => {
  ...
  expect((await app.request("/v1/assist", { method: "POST", headers: jsonHeaders, body: JSON.stringify(request) })).status).toBe(200);
  assist.mockClear();
  for (const headers of [
    new Headers(jsonHeaders),
    new Headers({ ...jsonHeaders, "Content-Length": String(MAX_BODY_BYTES + 1) }),
  ]) {
...
it("rejects server-side collection and string overflows before provider", async () => {
  ...
  for (const body of bodies) expect((await app.request("/v1/assist", { method: "POST", headers: jsonHeaders, body: JSON.stringify(body) })).status).toBe(400);
  expect(assist).not.toHaveBeenCalled();
});
it("bounds each logical operation even when timed-out provider ignores abort", async () => {
  ...
  for (let i = 0; i < 2; i++) expect((await app.request("/v1/assist", { method: "POST", headers: jsonHeaders, body: JSON.stringify(request) })).status).toBe(504);
  expect((await app.request("/v1/assist", { method: "POST", headers: jsonHeaders, body: JSON.stringify(request) })).status).toBe(504);
